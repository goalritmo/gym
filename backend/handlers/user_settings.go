...
func GetUserSettingsHandler(w http.ResponseWriter, r *http.Request) {
	...
	query := `
		SELECT show_own_workouts_in_social, unc_notifications_enabled
		FROM user_settings
		WHERE user_id = $1
	`
	...
	if err != nil {
		if err == sql.ErrNoRows {
			...
			insertQuery := `
				...
				VALUES ($1, $2, $3)
			`
			_, err = database.DB.Exec(insertQuery, userID, settings.ShowOwnWorkoutsInSocial, settings.UncNotificationsEnabled)
			if err != nil {
...
func UpdateUserSettingsHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, err := database.DB.Exec(query, userID, settings.ShowOwnWorkoutsInSocial, settings.UncNotificationsEnabled)
	if err != nil {
